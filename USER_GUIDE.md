...
启动参数按命令行参数解析，支持空格、引号和转义字符。比如 `-debug "D:/Celeste Saves/test profile"` 会作为两个参数传给游戏；如果路径或参数里有空格，请用引号包起来。
...
本地内容搜索会匹配名称、metadata 名称、版本、文件名、相对路径、作者、描述、SID、小图信息、依赖和警告。搜索结果会优先显示相关度更高的条目，并在名称或版本等可见字段上高亮命中的片段。

Mod 获取中心搜索也会匹配目录条目的名称、版本、类型、来源页面和 GameBanana 信息。搜索结果仍可继续叠加来源、类型、安装状态和可下载筛选。

例子：想找依赖某个 Helper 的地图或 Mod，可以直接搜索 Helper 名称；想找某个 SID 或文件路径，也可以直接输入其中一段。

## Mod 获取和更新

“下载 Mod”页用于搜索目录结果并安装新 Mod。安装或更新前，CelePkg 会先展示依赖树预览，标出已满足、缺失、版本不足、循环依赖、必需依赖和可选依赖。确认后才会加入下载任务。

“下载管理”页会显示下载中、下载失败、等待安装、安装成功和安装失败。下载和安装可以分别暂停、恢复、取消；失败项目可以重试。

取消或暂停发生在下载完成附近时，CelePkg 会清理尚未安装的暂存下载文件，避免留下无用 staged 文件。
...
本地内容详情里的“打开位置”只会打开当前 Celeste 目录下的 `Mods` 或官方 `Content` 目录内位置。备份管理里的“位置”只会打开当前 Celeste 备份根目录下的备份快照目录。

如果路径已经不存在、指向外部目录，或备份快照缺少 `manifest.json`，CelePkg 会拒绝打开。这样可以避免来自旧扫描结果或异常数据的任意路径打开。
