...
Profile 列表每一行右侧都有克隆按钮。它会完整克隆对应的 Profile，生成一个新 Profile 并自动选中。新名称会使用 `原名 Clone`，重名时会自动递增。
...
地图详情里的“小图”页会在列表上方显示小图搜索和排序选项。搜索会匹配小图名称和 SID；排序可按文件顺序、名称、完成状态、死亡数、用时或草莓数量切换，也可以倒序显示。

开启“按难度分组”时，小图会先按 Easy、Medium、Hard、高难等难度组归类，再在组内按当前排序方式排列。开启“仅搜索当前层级”时，搜索只匹配当前文件夹层级，不会进入子文件夹。

## Mod 获取和更新

“Mod 获取”页用于搜索目录结果并安装新 Mod。安装或更新前，CelePkg 会先展示依赖树预览，标出已满足、缺失、版本不足、循环依赖、必需依赖和可选依赖。确认后才会加入下载任务。

“下载管理”页会显示下载中、下载失败、等待安装、安装成功和安装失败。下载和安装可以分别暂停、恢复、取消；失败项目可以重试。左侧栏会显示下载任务的安装完成数、下载完成数和总数，方便在其他页面判断当前任务进度。
...
本地内容详情里的“打开所在位置”只会打开当前 Celeste 目录下的 `Mods` 或官方 `Content` 目录内位置。备份管理里的“位置”只会打开当前 Celeste 备份根目录下的备份快照目录。
