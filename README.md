...
CelePkg 是一个面向 Celeste 的本地 Mod / 地图管理器。它使用 Tauri 2 + React 构建，用来扫描 Celeste 的 `Mods` 目录，管理地图和其他 Mod 的启用状态，保存 Profile，并在修改游戏文件前自动备份。

## 功能

- 扫描 Celeste `Mods` 目录，识别地图包、其他 Mod、Helper 测试图和依赖关系。
- 读取地图存档统计，展示完成状态、死亡数、用时、草莓等信息。
