...
- 本地内容和 Mod 获取支持按名称、版本、文件名、相对路径、作者、描述、SID 和依赖等字段搜索，并高亮匹配片段。
- 草莓分母可切换：
  - 默认使用游戏概览分母。
...
- 左侧栏按“内容管理”“获取与安装”“维护”分组，并直接显示当前 Profile、启用数量、依赖 Mod 数和下载任务进度。
- 顶部支持“应用并启动”和“直接启动”：
...
- 支持受管文件备份与还原：
  ...
  - 可打开备份根目录或单个备份快照目录。
- 左侧“本地内容”统一承载地图和其他 Mod，列表内可在“地图 / 其他 Mod”之间切换；小图页内可按名称或 SID 搜索，并支持排序、难度分组和当前层级搜索。
- Mod 获取支持目录搜索、来源筛选、类型筛选、安装状态识别、安装前依赖树预览和下载任务管理。
- 地图和 Mod 详情页可直接打开所在位置，便于从详情页定位本地文件或目录。
- 删除、覆盖、安装、更新和依赖处理等高风险操作统一使用应用内确认弹窗，确认前会展示目标、版本、数量或风险说明。
- 支持普通刷新和“刷新缓存并重新扫描地图”，用于地图文件或缓存口径变化后的强制重扫。
- 前后端 API 契约由 Rust 边界类型生成，前端构建时使用生成的 TypeScript 类型和静态校验器，避免 Tauri CSP 下的运行时脚本求值。
