...
pub fn scan_mods(celeste_path: &Path, profiles: &ProfilesState) -> ScanResult {
    ...
    if !mods_path.exists() {
        return ScanResult {
            ...
            blacklist_path: mods_path.join("blacklist.txt").to_string_lossy().to_string(),
...
pub fn write_profile_blacklist(
    ...
    let mut file = File::create(&blacklist.file).map_err(|error| format!("写入 blacklist 失败：{error}"))?;
    let content = format!("{}\n", lines.join("\n").trim());
...
fn read_directory_mod(dir_path: &Path, mods_path: &Path) -> Option<ModRecord> {
    ...
    for entry in WalkDir::new(dir_path).into_iter().flatten() {
        ...
        let relative = normalize_slash(&entry.path().strip_prefix(dir_path).ok()?.to_string_lossy());
        ...
        if is_dialog_file(&relative) {
            dialog_texts.push((relative.clone(), fs::read_to_string(entry.path()).unwrap_or_default()));
        }
...
fn create_mod_record(
    ...
    let relative_path = normalize_slash(&absolute_path.strip_prefix(mods_path).unwrap_or(absolute_path).to_string_lossy());
    ...
    let sub_maps: Vec<SubMapInfo> = map_ids
        ...
        .map(|sid| SubMapInfo {
            ...
            sid: sid.clone(),
            display_name: dialog_title_for_sid(sid, &dialog_titles).unwrap_or_else(|| sub_map_display_name(sid)),
...
fn is_test_map_sid(sid: &str) -> bool {
    ...
    lower.split('/').any(|part| {
        matches!(part, "test" | "debug" | "sample" | "example" | "preview" | "dev" | "sandbox")
            || part.ends_with("test")
...
mod tests {
    ...
    fn classifies_code_mod_with_only_test_maps_as_other_mod() {
        ...
        let entries = vec!["Example.dll".to_string(), "Maps/preview/test.bin".to_string()];
    ...
    fn blacklist_writes_maps_and_mods_from_separate_enabled_sets() {
        let root = std::env::temp_dir().join(format!("celepkg-blacklist-test-{}", stable_id(&crate::utils::now_string())));
