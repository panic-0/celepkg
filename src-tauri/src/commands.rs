use crate::domain::{AppConfig, ConfigResponse, LaunchResult, ProfileInput, ProfilesState, ScanResult};
use crate::services;
...
pub async fn scan_celeste(celeste_path: String) -> Result<ScanResult, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        Ok(services::scan::full_scan(&path, state.profiles_state()))
    })
    .await
...
pub async fn apply_profile(celeste_path: String, profile_id: String) -> Result<ScanResult, String> {
    tauri::async_runtime::spawn_blocking(move || services::profile::apply_profile(celeste_path, profile_id))
        .await
        .map_err(|error| format!("应用 Profile 任务失败：{error}"))?
}

#[tauri::command]
pub async fn launch_profile(celeste_path: String, profile_id: String) -> Result<LaunchResult, String> {
    tauri::async_runtime::spawn_blocking(move || services::profile::launch_profile(celeste_path, profile_id))
        .await
